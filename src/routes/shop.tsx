...
type Shelf = { title: string; kind: "outfit" | "hat" | "pet"; price: number; items: Option[] };

const SHELVES: Shelf[] = [
  // The first two outfits are free starter clothes.
  { title: "Outfits", kind: "outfit", price: PRICES.outfit, items: OUTFITS.slice(2) },
  { title: "Hats", kind: "hat", price: PRICES.hat, items: HATS },
  { title: "Pet friends", kind: "pet", price: PRICES.pet, items: PETS },
];
...
function ShopPage() {
  ...
  return (
    <AppShell title="Coin shop" back={{ to: "/" }} right={<CoinPurse />}>
      <div className="mx-auto max-w-3xl">
        <section className="flex items-center gap-4 rounded-3xl border border-primary/20 bg-card/70 p-5">
          ...
          <div>
            <h2 className="text-xl font-extrabold">
              {look.name ? `${look.name}'s wardrobe` : "Your buddy's wardrobe"}
            </h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Earn 🪙 {REWARDS.star} per quiz star, 🪙 {REWARDS.chapterRead} for finishing a
              chapter, 🪙 {REWARDS.newWord} for each new word you tap, and 🪙 {STREAK_BONUS} for
        ...
        {SHELVES.map((shelf) => (
          <section key={shelf.kind} className="mt-7">
            <h3 className="text-lg font-extrabold">{shelf.title}</h3>
        ...
        <p className="mt-8 rounded-3xl bg-secondary/40 p-4 text-sm text-muted-foreground">
          Coins are just for fun — they are saved on this device and never cost real money. You can
          also spend 🪙 {PRICES.chapter} to open a locked chapter early, or 🪙 {PRICES.book} to make
          a brand new book.
        </p>
      </div>
    </AppShell>
